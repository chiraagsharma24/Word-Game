import React from 'react';
import { Button } from '@/components/ui/button';
import Link from 'next/link';
import { ThemeToggle } from '@/components/theme-toggle';
import { cn } from '@/lib/utils';

interface HeaderProps {
  onOpenStats: () => void;
}

export function Header({ onOpenStats }: HeaderProps) {
  return (
    <header className="backdrop-blur-md bg-background/30 border-b border-border/40 sticky top-0 z-10 w-full">
      <div className="max-w-5xl mx-auto flex justify-between items-center py-3 px-4">
        <div className="flex-1 flex justify-start gap-2">
          <Button variant="ghost" size="icon" asChild className="hover:bg-accent/10">
            <Link href="/how-to-play">
              <span className="text-lg">❓</span>
              <span className="sr-only">How to play</span>
            </Link>
          </Button>
          <ThemeToggle />
        </div>
        
        <Link href="/" className="flex-grow-0">
          <h1 className="text-2xl font-extrabold tracking-tight flex items-center gap-1.5">
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">Word</span>
            <span className="bg-gradient-to-r from-primary to-accent px-2 py-1 rounded-md text-white">Puzzle</span>
          </h1>
        </Link>
        
        <div className="flex-1 flex justify-end gap-2">
          <Button 
            variant="ghost" 
            size="icon" 
            onClick={onOpenStats}
            className="hover:bg-accent/10 hover:scale-105 transition-transform"
          >
            <span className="text-lg">📊</span>
            <span className="sr-only">Statistics</span>
          </Button>
          
          <Button
            variant="outline"
            size="sm"
            asChild
            className="hidden sm:flex text-xs bg-background/80 border-accent/20 hover:border-accent/40"
          >
            <Link href="https://github.com/yourusername/word-puzzle" target="_blank" rel="noopener noreferrer">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-1">
                <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"></path>
                <path d="M9 18c-4.51 2-5-2-7-2"></path>
              </svg>
              <span>GitHub</span>
            </Link>
          </Button>
        </div>
      </div>
    </header>
  );
} 